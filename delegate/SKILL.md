...
Use `delegate.py` to dispatch heavy LLM work via the opencode CLI instead of burning your own context window.
delegate.py is on the PATH and needs to be run outside the sandbox.

## Commands

### ask — bulk file analysis
```bash
./delegate.py ask --paths <file>... --question "<question>"
```
...
```bash
./delegate.py write --spec "<what to generate>" --target <output> [--context <ref>...] [--overwrite]
```
